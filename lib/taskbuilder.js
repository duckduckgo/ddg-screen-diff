...
function getTasks(ops) {
    ...
    // if not, error out
    var hasEnoughInfoForDiff = ops.hosts.length === 2 ||
            // the url command requires a bit more flexibility, because
            // we're comparing arbitrary URLs against each other
            ops.command === "url";

    if (ops.diff && !hasEnoughInfoForDiff) {
