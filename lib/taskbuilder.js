        ...
        https.get("https://duck.co/ia/view/" + iaName + "/json", function (res) {
            ...
            res.on("end", function () {
                ...
                try {
                    ...
                    return reject(new Error("couldn't get metadata for IA " + iaName + " - does it exist?"));
                }

                if (data.live) {
                    console.log("got metadata for IA " + iaName);
                    resolve(data.live);
                } else {
                    reject(new Error("IA " + iaName + " doesn't seem to be live"));
                }
            });
        }).on("error", reject);
    });
