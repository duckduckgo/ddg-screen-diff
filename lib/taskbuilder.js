...
function getMetadata(iaName) {
    return getAllMetadataFromServer()
...
function getAllMetadataFromServer() {
    if (cachedMetadata) {
        return Promise.resolve(cachedMetadata);
    }

    return new Promise(function (resolve, reject) {
        https.get("https://duck.co/ia/repo/all/json", function (res) {
            var dataJSON = "";
            ...
            res.on("end", function () {
                ...
                try {
                    ...
                    return reject(new Error("couldn't parse metadata JSON from server: " + e.message));
                }
