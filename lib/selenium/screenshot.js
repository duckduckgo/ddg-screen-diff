...
function tryAndLoadUrl(url, driver, deferred) {
    ...
    // Since PhantomJS is a touch device, it shows scrollbars in our
    // Instant Answers. It looks like we can't execute Modernizr.touch = false
    // before anything else (https://stackoverflow.com/questions/31354352/selenium-how-to-inject-execute-a-javascript-in-to-a-page-before-loading-executi)
...
function getDriver(task, cachedDrivers) {
    ...
    if (browserInfo.browser !== "phantomjs") {
        ...
        // allow overriding of the useragent
        if (config.userAgent) {
            capabilities["phantomjs.page.settings.userAgent"] = config.userAgent;
        }
