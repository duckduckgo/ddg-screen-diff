    ...
    return new until.Condition("for page to finish loading", function (driver) {
        ...
        return driver.executeScript(function () {
            if (window.DDG &&
                ...
                return true;//document.readyState === "loaded" || document.readyState === "complete" || document.readyState === "interactive";
            }
...
function tryAndLoadUrl(url, driver, deferred) {
    ...
    driver.wait(untilPageHasLoaded(driver)).then(function success() {
            deferred.fulfill();
...
function getDriver(task, cachedDrivers) {
    ...
    if (!/headless-chromium/.test(browserInfo.browser)) {
        ...
        cachedDrivers.xvfb = new Xvfb({ reuse: false, xvfb_args: [
            "-screen",
            "0",
            task.size.width + "x" + task.size.height + "x16"
        ]});
        cachedDrivers.xvfb.startSync();
