...
function cropScreenshot(task) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        .write(task.imgPath, function (err) {
            if (err) {
                task.error = "couldn't save image";
            }
            resolve(task);
        });
    });
...
function generateDiffs(tasks) {
    ...
    for (var i = 0; i < tasks.length; i += 2) {
        // if any of the tasks errored out, bail on generating a diff
        ...
        // promisify doesn't work with compare for some reason,
        // wrap in a promise manually
        var promise = new Promise(function (resolve, reject) {
            ...
            gm.compare(task1.imgPath, task2.imgPath, {
                ...
                if (err) { console.log("couldn't create diff:", diffPath); }

                console.log(arguments);
                task1.eq = task2.eq = equality;
                resolve();
            })
        });
...
function getTaskDescription(task) {
    var description = task.url;

    if (task.browser.indexOf("headless-chromium") === -1) {
        description += " on " + task.browser;
    }
...
function createPage(tasks) {
    ...
    if (shouldDiff) {
        for (i = 0; i < tasks.length; i += 2) {
            ...
            markup += "<td>" + getTaskDescription(tasks[i]) + "</td>";
            markup += "<td>" + getTaskDescription(tasks[i + 1]) + "</td>";
            markup += "</tr>";
