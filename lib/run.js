...
createScreenshotDir().then(function () {
    ...
    // logs for phantomjs are dumped in the cwd, so this makes sure
    // they only appear in the screenshot folder
...
function generateDiffs(tasks) {
    ...
    for (var i = 0; i < tasks.length; i += 2) {
        ...
        diffPromises.push(gm.compareAsync(tasks[i].imgPath, tasks[i + 1].imgPath, {
            file: diffPath,
